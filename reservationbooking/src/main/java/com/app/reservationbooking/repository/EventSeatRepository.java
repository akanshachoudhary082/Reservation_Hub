...
/**
 * Repository interface for managing Seat entities related to events.
 */
public interface EventSeatRepository extends JpaRepository<Seat, Long> {
     ...
     * @param showName the name of the event or show
     ...
     */
    @Query("SELECT a FROM Seat a WHERE a.city = ?1 AND a.showName = ?2 AND a.theaterName = ?3 AND a.showDateTime = ?4 ORDER BY seatNumber ASC")
