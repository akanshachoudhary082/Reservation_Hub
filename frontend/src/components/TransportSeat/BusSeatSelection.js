...
//           {/* Redirect to booking page with selected seat data */}
//           {/* <Button
//             variant="contained"
//             color="primary"
//             sx={{ mt: 3 }}
//             onClick={() => navigate('/transport-booking', { state: { selectedSeats, totalPrice } })} // Pass selectedSeats and totalPrice to booking page
//           > */}
//           <Button
//                       variant="contained"
//                       color="primary"
...
const BusSeatSelection = ({ upperDeck = [], lowerDeck = [] }) => {
  ...
  const handleSelect = (seat) => {
    ...
    const selectedObjs = allSeats.filter(s => updatedSeats.includes(s.seatId));

    setSelectedSeats(updatedSeats);
    setSelectedSeatObjects(selectedObjs);
    setSeatCount(updatedSeats.length);

    const updatedPrice = selectedObjs.reduce((sum, s) => sum + (s.seatPrice || 0), 0);
    setTotalPrice(updatedPrice);
  ...
  const toggleAddRemove = () => {
    ...
    if (isAddMode && selectedSeats.length > 0) {
      const updatedSeats = selectedSeats.slice(0, -1);
      const updatedObjs = allSeats.filter(seat => updatedSeats.includes(seat.seatId));

      setSelectedSeats(updatedSeats);
      setSelectedSeatObjects(updatedObjs);
      setSeatCount(updatedSeats.length);

      const updatedPrice = updatedObjs.reduce((sum, s) => sum + (s.seatPrice || 0), 0);
      setTotalPrice(updatedPrice);
    }
  };
  ...
  const renderSelectedSeatNumbers = () =>
    selectedSeatObjects.map(seat => seat.seatNumber).join(', ');
